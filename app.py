...
from data.data import changes, projects, updates, activities, cost_loading
...
def index():
    ...
    if request.method == "POST":
        if (curr := request.files.get('current')) and (prev := request.files.get('previous')):
            ...
            return render_template('home/results.html', changes=changes, updates=updates, projects=projects, activities=activities, cost=cost_loading)
